            ...
            if answer == request.session['answer']:
                ...
                if check_blackuser:
                    ...
                    if product_detail:
                        ...
                        setcache(product_key, 60 * 5, product_detail_to)
                    ...
                    if diffstarttime.days >= 0 and expiretime < EXPIREMAXTIME:
                        ...
                        if check_request_limit(ua_key, REQUEST_LIMITMAX):
                            ...
                            # 定义队列key
                            queue_product_key = 'queue_' + product_key
                            # 取出当时缓存库存
                            stock_qty = int(getcache(key_stock))
                            # 检查队列大小，如不符合，则不进队列
                            # 取队列大小
                            total_quesize = queuesize(queue_product_key)
                            ...
                            if total_quesize > 2 * stock_qty:
                                ...
                                # 入队列排队
                                setqueue(queue_product_key, que_user)
                            ...
                            msg = '请求次数超过{}次'.format(REQUEST_LIMITMAX)
                            status = 300
                            orderid = -1
                    else:
                        msg = '时间不正确'
        ...
        if order.status == '2':
            try:
                if pay_type == 1:
                    ...
                    url = "/wxpaying?order_id=" + str(order_id) + "&result_src=" + result_src + "&amount=" + str(
                        realpayamount)
        ...
        if dicwxBack and dicwxBack.has_key("return_code"):
            if dicwxBack["return_code"] == "SUCCESS":
                if dicwxBack["result_code"] == "SUCCESS":
                    ...
                    dicBack['PAYTYPE'] = 1  # '订单'
                    dicBack['PAYSOURCE'] = 2  # '微信支付'
                    dicBack['PAYSTATUS'] = 3  # '支付
...
def pay_success_commit_corder(request, order_id, dicBack):
    ...
    :param corder:
    :param dicBack:
    orderSTATUS = (('1', '生成'),('0', '取消'), ('2', '待支付'),('3', '已支付'),('4', '退款'), ('5', '评价'),)
    :return:
    """
    flag = False
    ...
    if order.amount == dicBack["total_amount"]:
        with transaction.atomic():
            if order.status == '2':
                reply_dump = '交易单号：' + dicBack["trade_no"] + '支付类型：' + dicBack["trade_type"] + '支付金额：' + str(
                    dicBack["total_amount"]) + '支付人：' + dicBack['auth_token']
                reference_number = dicBack["trade_no"]
                # 1、业务 更新订单状态和交易号、交易时间等信息。
                order = Order.objects.filter(id=int(order_id)).update(reply_dump=reply_dump,
                                                                      reference_number=reference_number, status='3')
                return True, order
            else:
                return False
...
def alipay_success(request):
    ...
    if alipay_response(request.GET):
        ...
        dicBack['PAYTYPE'] = 1  # '订单'
        dicBack['PAYSOURCE'] = 4  # '支付宝支付'
        dicBack['PAYSTATUS'] = 1  # '支付'
